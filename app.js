// ===== Configuration =====
const API_URL = 'https://agridrop-vxci.onrender.com'; // Your specific Render URL
let myChart = null;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const prevBtn = document.querySelector('.prev'); // Matches your HTML class
    const nextBtn = document.querySelector('.next'); // Matches your HTML class
    ...
    function showSlide(index) {
        ...
        // Moves the slides horizontally based on your CSS flexbox setup
        slidesContainer.style.transform = `translateX(-${index * 100}%)`;
        currentIndex = index;
    }

    // Manual Clicks for Arrows
    prevBtn.addEventListener('click', () => showSlide(currentIndex - 1));
    nextBtn.addEventListener('click', () => showSlide(currentIndex + 1));

    // ===== 2. Page Transition =====
    const enterAppBtn = document.getElementById('enterApp'); // Matches your HTML ID
    const sliderContainer = document.querySelector('.slider-container');
    const appContainer = document.querySelector('.app-container');

    enterAppBtn.addEventListener('click', () => {
        sliderContainer.style.display = 'none';
        enterAppBtn.style.display = 'none'; // Hide the green button
        appContainer.style.display = 'block'; // Show the form
    });
    ...
    cropForm.addEventListener('submit', async (e) => {
        ...
        try {
            // Updated to match your backend's expected parameters (?water=&region=&land=)
            ...
            // Print detailed results as you requested
            ...
            resultsDiv.innerHTML = `<p style="color:red;">Error fetching data. Ensure the Render server is running.</p>`;
        }
    });
});
